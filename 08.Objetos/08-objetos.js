...
function permisosDeEntrada(personas) {
    ...
    // tu código:
    let personasPermitidas = personas.filter(persona => persona.entrada && persona.edad >= 18);
    let nombresPermitidos = personasPermitidas.map(persona => persona.nombre);
    return nombresPermitidos;
}
...
function informacionLibro(libro) {
  ...
  function mostrarInformacionLibro(libro) {
    console.log(`El libro ${libro.titulo}, escrito por ${libro.autor}, fue publicado en ${libro.añoPublicacion}.`);
}
